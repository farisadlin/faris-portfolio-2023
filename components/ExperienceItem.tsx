...
const ExperienceItem: React.FC<ExperienceItemProps> = ({ experience }) => {
    ...
    return (
      <div className="flex flex-col sm:relative sm:before:absolute sm:before:top-2 sm:before:w-4 sm:before:h-4 sm:before:rounded-full sm:before:left-[-35px] sm:before:z-[1] before:dark:bg-secondary">
        ...
        {jobPosition} @ <a target="_blank" rel="noopener noreferrer" href={url} className="text-secondary hover:underline cursor-pointer">{company}</a>
        </h3>
        <time className="text-xs tracking-wide uppercase dark:text-gray-400 font-space-mono">{dateRange}</time>
        <ul className="mt-3">
