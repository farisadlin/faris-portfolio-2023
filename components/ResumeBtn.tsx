...
const ResumeBtn = ({noMargin = false, text = 'Resume', urlDownload = '#', hasResume = true}) => {
  const downloadUrl = () => window.open('/portfolio_faris-adlin.pdf', '_blank');
  return (
    <a href={urlDownload} className={`relative inline-block text-lg group font-space-mono ${noMargin ? '' : 'mt-12'}`} onClick={hasResume ? downloadUrl : null}>
