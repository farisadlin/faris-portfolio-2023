...
const Experiences = () => {
  ...
  return (
    <section className='h-screen xl:mx-40 py-28' id='experience'>
    ...
    <div className='flex flex-col md:flex-row mt-10'>
      <div className='overflow-x-auto md:overflow-x-visible flex mb-10 flex-row md:flex-col md:mb-0'>
        {EXPERIENCES.map(experience => <button
          ...
          className={`text-start md:w-48 px-6 py-4 font-fira-code ${dataExperiences.id === experience.id ? 'bg-slate-700 text-secondary border-secondary' : 'border-stone-700'} border-b-2 md:border-b-0 md:border-l-2 duration-150`}>
          {experience.company}
        </button>)}
      </div>
      <div className='ml-5'>
        <div className='text-xl text-zinc-300'>
          ...
          <a className='text-secondary hover:underline cursor-pointer' href={dataExperiences.url} target='_blank' rel='noopener'>{dataExperiences.company}</a>
        </div>
